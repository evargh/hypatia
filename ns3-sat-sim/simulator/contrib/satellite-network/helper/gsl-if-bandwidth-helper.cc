...
void GslIfBandwidthHelper::UpdateGslIfBandwidth(int64_t t)
{
	...
	res << m_basicSimulation->GetConfigParamOrFail("satellite_network_routes_dir") << "/gsl_if_bandwidth_" << t
		<< ".txt";
	...
	if (fstate_file)
	{
		...
		while (getline(fstate_file, line))
		{
			...
			// Check the interface
			NS_ABORT_MSG_IF(if_id < 0 || if_id + 1 >= m_nodes.Get(node_id)->GetObject<Ipv4>()->GetNInterfaces(),
							"Invalid interface");

			// Set data rate
			Ptr<NetDevice> nd = m_nodes.Get(node_id)->GetObject<Ipv4>()->GetNetDevice(1 + if_id);
			if (nd->GetObject<GSLNetDevice>() != 0)
			{
				nd->GetObject<GSLNetDevice>()->SetDataRate(
					DataRate(std::to_string(m_gsl_data_rate_megabit_per_s * bandwidth_fraction) + "Mbps"));
			}
			else
			{
				NS_ASSERT_MSG(false, "no GSLNetDevice found");
			}
	...
	if (!parse_boolean(m_basicSimulation->GetConfigParamOrDefault("satellite_network_force_static", "false")))
	{
		...
		if (next_update_ns < m_basicSimulation->GetSimulationEndTimeNs())
		{
			Simulator::Schedule(NanoSeconds(m_dynamicStateUpdateIntervalNs),
								&GslIfBandwidthHelper::UpdateGslIfBandwidth, this, next_update_ns);
		}
	}
}
