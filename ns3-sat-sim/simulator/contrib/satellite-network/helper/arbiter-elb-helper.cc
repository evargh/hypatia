...
#include "arbiter-elb-helper.h"

namespace ns3
{
NS_LOG_COMPONENT_DEFINE("ArbiterElbHelper");

ArbiterElbHelper::ArbiterElbHelper(Ptr<BasicSimulation> basicSimulation, NodeContainer nodes)
{
	std::cout << "SETUP ELB ROUTING" << std::endl;
	...
	int64_t gsl_queue_size_packets =
	...
	adjacent_satellite_table.resize(m_nodes.GetN() - num_satellites);
	FlowVec sspf;
	sspf.resize(m_nodes.GetN() - num_satellites);
	for (auto &elem : sspf)
	{
		elem.resize(m_nodes.GetN() - num_satellites);
	}
	source_satellite_per_flow = std::unique_ptr<ArbiterElbHelper::FlowVec>(new ArbiterElbHelper::FlowVec(sspf));

	std::vector<std::tuple<int8_t, double>> stack_vector;
	...
	shared_data_for_satellites = std::shared_ptr<std::vector<std::tuple<int8_t, double>>>(
		new std::vector<std::tuple<int8_t, double>>(stack_vector));
	...
	for (int32_t i = 0; i < num_satellites; i++)
	{
		...
		Ptr<ArbiterElbSat> arbiter = CreateObject<ArbiterElbSat>(
			...
			right_neighbor_gamma_difference, isl_queue_size_packets, gsl_queue_size_packets,
			ArbiterElbHelper::ELB_UPDATE_INTERVAL_NS);
...
std::vector<std::tuple<int32_t, int32_t, int32_t>> ArbiterElbHelper::CreateOutboundInterfaceList(int32_t i)
{
...
std::tuple<double, double, double, double> ArbiterElbHelper::CartesianToShort(Vector3D cartesian)
{
	Vector2D latlon = Vector2D(std::asin(cartesian.z / ArbiterElbHelper::APPROXIMATE_EARTH_RADIUS_M) * 180.0 / pi,
...
void ArbiterElbHelper::SetCoordinateSkew()
{
...
std::vector<std::vector<std::tuple<int32_t, int32_t, int32_t>>> ArbiterElbHelper::InitialEmptyForwardingState()
{
	std::vector<std::vector<std::tuple<int32_t, int32_t, int32_t>>> initial_forwarding_state;
...
void ArbiterElbHelper::UpdateOrbitalParams(int64_t t)
{
	// Filename
	std::ostringstream res;
	res << m_basicSimulation->GetRunDir() << "/";
	...
	if (!parse_boolean(m_basicSimulation->GetConfigParamOrDefault("satellite_network_force_static", "false")))
	{
		...
		{
			Simulator::Schedule(NanoSeconds(m_dynamicStateUpdateIntervalNs), &ArbiterElbHelper::UpdateOrbitalParams,
								this, next_update_ns);
		}
	}
}

void ArbiterElbHelper::UpdateForwardingState(int64_t t)
{
	...
	if (fstate_file)
	{
		...
		while (getline(fstate_file, line))
		{
			...
			else
			{
				...
				source_satellite_per_flow->at(current_node_id - m_num_orbits * m_satellites_per_orbit)
					.at(target_node_id - m_num_orbits * m_satellites_per_orbit) =
					std::make_tuple(next_hop_node_id, satellite_positions_short.at(next_hop_node_id));
			}
		}
		...
		for (int current_node_id = 0; current_node_id < m_num_orbits * m_satellites_per_orbit; current_node_id++)
		{
			...
			m_sat_arbiters.at(current_node_id)->SetSourceSatelliteTable(source_satellite_per_flow.get());
		}
		// Close file
		fstate_file.close();
	}
	...
	if (!parse_boolean(m_basicSimulation->GetConfigParamOrDefault("satellite_network_force_static", "false")))
	{
		...
		if (next_update_ns < m_basicSimulation->GetSimulationEndTimeNs())
		{
			Simulator::Schedule(NanoSeconds(m_dynamicStateUpdateIntervalNs), &ArbiterElbHelper::UpdateForwardingState,
								this, next_update_ns);
		}
	}
}

void ArbiterElbHelper::UpdateBusyLevels(int16_t t)
{
	// every time this runs, iterate through the list of satellites and make the appropriate calculations
	for (auto &elem : m_sat_arbiters)
	{
		elem->UpdateELBState();
	}
	...
	{
		// Plan the next update
		int64_t next_update_ns = t + ELB_UPDATE_INTERVAL_NS;
		if (next_update_ns < m_basicSimulation->GetSimulationEndTimeNs())
		{
			Simulator::Schedule(NanoSeconds(ELB_UPDATE_INTERVAL_NS), &ArbiterElbHelper::UpdateBusyLevels, this,
								next_update_ns);
		}
	}
}
