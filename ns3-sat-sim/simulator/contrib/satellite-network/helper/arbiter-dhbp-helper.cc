...
ArbiterDhbpHelper::ArbiterDhbpHelper(Ptr<BasicSimulation> basicSimulation, NodeContainer nodes)
{
	...
	source_satellite_per_flow = std::unique_ptr<ArbiterDhbpHelper::FlowVec>(new ArbiterDhbpHelper::FlowVec(sspf));

	std::vector<std::vector<int64_t>> stack_vector;
	...
	shared_data_for_satellites =
		std::shared_ptr<std::vector<std::vector<int64_t>>>(new std::vector<std::vector<int64_t>>(stack_vector));
