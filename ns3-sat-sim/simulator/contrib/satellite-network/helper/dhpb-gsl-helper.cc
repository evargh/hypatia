...
NS_LOG_COMPONENT_DEFINE ("DhpbGSLHelper");

DhpbGSLHelper::DhpbGSLHelper ()
{
  m_queueFactory.SetTypeId ("ns3::DropTailQueue<Packet>");
  m_deviceFactory.SetTypeId ("ns3::DhpbGSLNetDevice");
  m_channelFactory.SetTypeId ("ns3::GSLChannel");
}

void 
DhpbGSLHelper::SetQueue (std::string type,
...
void 
DhpbGSLHelper::SetDeviceAttribute (std::string n1, const AttributeValue &v1)
{
  m_deviceFactory.Set (n1, v1);
}

void 
DhpbGSLHelper::SetChannelAttribute (std::string n1, const AttributeValue &v1)
{
  m_channelFactory.Set (n1, v1);
}

NetDeviceContainer 
DhpbGSLHelper::Install (NodeContainer satellites, NodeContainer ground_stations, std::vector<std::tuple<int32_t, double>>& node_gsl_if_info)
{
...
Ptr<DhpbGSLNetDevice>
DhpbGSLHelper::Install (Ptr<Node> node, Ptr<GSLChannel> channel) {

    // Create device
    Ptr<DhpbGSLNetDevice> dev = m_deviceFactory.Create<DhpbGSLNetDevice>();
    ...
    mpiRec->SetReceiveCallback (MakeCallback (&DhpbGSLNetDevice::Receive, dev));
