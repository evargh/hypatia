...
void ArbiterShortHelper::UpdateForwardingState(int64_t t)
{
	...
	if (fstate_file)
	{
		...
		while (getline(fstate_file, line))
		{
			...
			if (next_hop_node_id != -1 && my_if_id != -1 && next_if_id != -1)
			{
				...
				bool source_is_gsl =
						m_nodes.Get(current_node_id)->GetObject<Ipv4>()->GetNetDevice(1 + my_if_id)->GetObject<GSLNetDevice>() != 0;
	...
	if (!parse_boolean(m_basicSimulation->GetConfigParamOrDefault("satellite_network_force_static", "false")))
	{
		...
		{
			Simulator::Schedule(NanoSeconds(m_dynamicStateUpdateIntervalNs), &ArbiterShortHelper::UpdateForwardingState, this,
													next_update_ns);
		}
	}
}
