...
DhpbGSLNetDevice::DhpbGSLNetDevice() : m_txMachineState(READY), m_channel(0), m_linkUp(false), m_currentPkt(0)
{
...
void DhpbGSLNetDevice::AddHeader(Ptr<Packet> p, uint16_t protocolNumber)
{
...
bool DhpbGSLNetDevice::ProcessHeader(Ptr<Packet> p, uint16_t &param)
{
...
void DhpbGSLNetDevice::DoDispose()
{
	NS_LOG_FUNCTION(this);
	m_node = 0;
	m_channel = 0;
...
void DhpbGSLNetDevice::SetDataRate(DataRate bps)
{
	NS_LOG_FUNCTION(this);
	m_bps = bps;
}

void DhpbGSLNetDevice::SetInterframeGap(Time t)
{
	NS_LOG_FUNCTION(this << t.GetSeconds());
	m_tInterframeGap = t;
}

bool DhpbGSLNetDevice::TransmitStart(Ptr<Packet> p, const Address dest)
{
...
void DhpbGSLNetDevice::TransmitComplete(const Address dest)
{
	NS_LOG_FUNCTION(this);
...
bool DhpbGSLNetDevice::Attach(Ptr<GSLChannel> ch)
{
...
void DhpbGSLNetDevice::SetQueue(Ptr<Queue<Packet>> q)
{
	NS_LOG_FUNCTION(this << q);
	m_queue = q;
}

void DhpbGSLNetDevice::SetReceiveErrorModel(Ptr<ErrorModel> em)
{
	NS_LOG_FUNCTION(this << em);
	m_receiveErrorModel = em;
}

void DhpbGSLNetDevice::Receive(Ptr<Packet> packet)
{
...
Ptr<Queue<Packet>> DhpbGSLNetDevice::GetQueue(void) const
{
	NS_LOG_FUNCTION(this);
	return m_queue;
}

void DhpbGSLNetDevice::NotifyLinkUp(void)
{
	NS_LOG_FUNCTION(this);
	m_linkUp = true;
...
void DhpbGSLNetDevice::SetIfIndex(const uint32_t index)
{
	NS_LOG_FUNCTION(this);
	m_ifIndex = index;
}

uint32_t DhpbGSLNetDevice::GetIfIndex(void) const
{
	return m_ifIndex;
}

Ptr<Channel> DhpbGSLNetDevice::GetChannel(void) const
{
	return m_channel;
}

void DhpbGSLNetDevice::SetAddress(Address address)
{
	NS_LOG_FUNCTION(this << address);
	m_address = Mac48Address::ConvertFrom(address);
}

Address DhpbGSLNetDevice::GetAddress(void) const
{
	return m_address;
}

bool DhpbGSLNetDevice::IsLinkUp(void) const
{
	NS_LOG_FUNCTION(this);
	return m_linkUp;
}

void DhpbGSLNetDevice::AddLinkChangeCallback(Callback<void> callback)
{
	NS_LOG_FUNCTION(this);
	m_linkChangeCallbacks.ConnectWithoutContext(callback);
}

bool DhpbGSLNetDevice::IsBroadcast(void) const
{
	NS_LOG_FUNCTION(this);
	return true; // We return true such that the normal Internet stack can be installed, because ARP needs true here
}

Address DhpbGSLNetDevice::GetBroadcast(void) const
{
	NS_LOG_FUNCTION(this);
...
Address DhpbGSLNetDevice::GetMulticast(Ipv4Address multicastGroup) const
{
	NS_LOG_FUNCTION(this);
	throw std::runtime_error("Multicast not supported.");
}

Address DhpbGSLNetDevice::GetMulticast(Ipv6Address addr) const
{
	NS_LOG_FUNCTION(this << addr);
	throw std::runtime_error("Multicast not supported.");
}

bool DhpbGSLNetDevice::IsPointToPoint(void) const
{
	NS_LOG_FUNCTION(this);
	return false;
}

bool DhpbGSLNetDevice::IsBridge(void) const
{
	NS_LOG_FUNCTION(this);
	return false;
}

bool DhpbGSLNetDevice::Send(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
...
bool DhpbGSLNetDevice::SendFrom(Ptr<Packet> packet, const Address &source, const Address &dest, uint16_t protocolNumber)
{
	NS_LOG_FUNCTION(this << packet << source << dest << protocolNumber);
	return false;
}

Ptr<Node> DhpbGSLNetDevice::GetNode(void) const
{
	return m_node;
}

void DhpbGSLNetDevice::SetNode(Ptr<Node> node)
{
	NS_LOG_FUNCTION(this);
	m_node = node;
}

bool DhpbGSLNetDevice::NeedsArp(void) const
{
	NS_LOG_FUNCTION(this);
	return true;
}

void DhpbGSLNetDevice::SetReceiveCallback(NetDevice::ReceiveCallback cb)
{
	m_rxCallback = cb;
}

void DhpbGSLNetDevice::SetPromiscReceiveCallback(NetDevice::PromiscReceiveCallback cb)
{
	m_promiscCallback = cb;
}

bool DhpbGSLNetDevice::SupportsSendFrom(void) const
{
	NS_LOG_FUNCTION(this);
	return false;
}

void DhpbGSLNetDevice::DoMpiReceive(Ptr<Packet> p)
{
	NS_LOG_FUNCTION(this << p);
	Receive(p);
}

bool DhpbGSLNetDevice::SetMtu(uint16_t mtu)
{
...
uint16_t DhpbGSLNetDevice::GetMtu(void) const
{
	NS_LOG_FUNCTION(this);
	return m_mtu;
}

uint16_t DhpbGSLNetDevice::PppToEther(uint16_t proto)
{
...
uint16_t DhpbGSLNetDevice::EtherToPpp(uint16_t proto)
{
	NS_LOG_FUNCTION_NOARGS();
	switch (proto)
	{
