...
#include "ipv4-dhpb-arbiter-routing.h"

namespace ns3
{

NS_LOG_COMPONENT_DEFINE("Ipv4DhpbArbiterRouting");

NS_OBJECT_ENSURE_REGISTERED(Ipv4DhpbArbiterRouting);
...
Ipv4DhpbArbiterRouting::Ipv4DhpbArbiterRouting() : m_ipv4(0)
{
																										 ...
																										 Ptr<const Packet> p, Ptr<NetDevice> oif)
{
	...
	if (loopbackMask.IsMatch(dest, loopbackIp))
	{ // Loop-back
		if_idx = 0;
		gateway_ip_address = 0;
	}
	else
	{ // If not loop-back, it goes to the arbiter
																				...
																				LocalDeliverCallback lcb, ErrorCallback ecb)
{
	...
	{ // WeakESModel is set by default to true,
...
Ipv4DhpbArbiterRouting::~Ipv4DhpbArbiterRouting()
{
	NS_LOG_FUNCTION(this);
}

void Ipv4DhpbArbiterRouting::NotifyInterfaceUp(uint32_t i)
{
...
void Ipv4DhpbArbiterRouting::NotifyInterfaceDown(uint32_t i)
{
	throw std::runtime_error("Interfaces are not permitted to go down.");
}

void Ipv4DhpbArbiterRouting::NotifyAddAddress(uint32_t interface, Ipv4InterfaceAddress address)
{
...
void Ipv4DhpbArbiterRouting::NotifyRemoveAddress(uint32_t interface, Ipv4InterfaceAddress address)
{
	if (m_ipv4->IsUp(interface))
	{
...
void Ipv4DhpbArbiterRouting::SetIpv4(Ptr<Ipv4> ipv4)
{
...
void Ipv4DhpbArbiterRouting::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
	std::ostream *os = stream->GetStream();
	*os << m_arbiter->StringReprOfForwardingState();
}

void Ipv4DhpbArbiterRouting::SetArbiter(Ptr<ArbiterDhpb> arbiter)
{
	m_arbiter = arbiter;
}

Ptr<ArbiterDhpb> Ipv4DhpbArbiterRouting::GetArbiter()
{
	return m_arbiter;
}

void Ipv4DhpbArbiterRouting::IncreaseArbiterDistance(Ptr<Packet> p)
{
	m_arbiter->AddQueueDistance(ResolveNodeIdFromPacket(p));
	NS_LOG_DEBUG("added packet to arbiter array");
}

void Ipv4DhpbArbiterRouting::ReduceArbiterDistance(Ptr<Packet> p)
{
	m_arbiter->ReduceQueueDistance(ResolveNodeIdFromPacket(p));
	NS_LOG_DEBUG("removed packet from arbiter array");
}

uint32_t Ipv4DhpbArbiterRouting::ResolveNodeIdFromPacket(Ptr<Packet> p)
{
	Ipv4Header ipHeader;
