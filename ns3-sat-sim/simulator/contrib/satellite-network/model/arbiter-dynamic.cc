...
#include "arbiter-dynamic.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("ArbiterDynamic");
NS_OBJECT_ENSURE_REGISTERED (ArbiterDynamic);
TypeId ArbiterDynamic::GetTypeId (void)
{
    static TypeId tid = TypeId ("ns3::ArbiterDynamic")
...
Vector3D 
ArbiterDynamic::CartesianToGeodetic(Vector3D cartesian) {
    return Vector3D(std::asin(cartesian.z/ArbiterDynamic::APPROXIMATE_EARTH_RADIUS_M)*180.0/pi, std::atan2(cartesian.y, cartesian.x)*180.0/pi, 0); 
}  

Ptr<Satellite>
ArbiterDynamic::ExtractSatellite(int32_t node_id) {
...
bool ArbiterDynamic::CheckIfInRectangle(
    Vector3D forwardpos,
    Vector3D srcpos, 
    Vector3D destpos
) {
...
Ptr<Satellite> ArbiterDynamic::GetClosestSatellite(std::set<int32_t> *node_id_set) {
...
Ptr<Satellite> ArbiterDynamic::GetFarthestSatellite(std::set<int32_t> *node_id_set) {
...
int32_t ArbiterDynamic::GSLNodeIdToGSLIndex(int32_t id) {
    return id - m_nodes.GetN() + NUM_GROUND_STATIONS;
}

int32_t ArbiterDynamic::GSLIndexToGSLNodeId(int32_t id) {
    return m_nodes.GetN() - NUM_GROUND_STATIONS + id;
}

double ArbiterDynamic::ValidateForwardHeuristic(
...
void ArbiterDynamic::GetNeighborInfo() {
...
std::tuple<int32_t, int32_t, int32_t> ArbiterDynamic::GetForward(
        int32_t source_node_id,
        int32_t target_node_id,
    ...
    std::sort(satellite_forwarding_information.begin(), satellite_forwarding_information.end(), 
                  [](std::tuple<uint64_t, int8_t, double> a,
                  std::tuple<uint64_t, int8_t, double> b)
                  {
    ...
    return std::make_tuple(
		m_neighbor_ids.at(optimal_outbound_interface),
		optimal_outbound_interface+1,
		m_neighbor_interfaces.at(optimal_outbound_interface)
    );
}

std::tuple<int32_t, int32_t, int32_t> ArbiterDynamic::TopologySatelliteNetworkDecide(
        int32_t source_node_id,
        int32_t target_node_id,
    ...
    NS_ASSERT(m_nodes.GetN() > ArbiterDynamic::NUM_GROUND_STATIONS);
...
void ArbiterDynamic::AddQueueDistance(int32_t target_node_id) {
...
uint64_t ArbiterDynamic::GetQueueDistance(int32_t target_node_id) {
    NS_ASSERT(GSLNodeIdToGSLIndex(target_node_id) < (int32_t)m_queueing_distances.size());
...
void ArbiterDynamic::ReduceQueueDistance(int32_t target_node_id) {
    NS_ASSERT(GSLNodeIdToGSLIndex(target_node_id) < (int32_t)m_queueing_distances.size());
...
void ArbiterDynamic::SetSingleForwardState(int32_t target_node_id, int32_t next_node_id, int32_t own_if_id, int32_t next_if_id) {
...
void ArbiterDynamic::ModifyDistanceLookup(int32_t target_node_id, uint32_t distance) {
    NS_ASSERT(target_node_id >= 0);
    NS_ASSERT(GSLNodeIdToGSLIndex(target_node_id) < (int32_t)m_queueing_distances.size());
    m_distance_lookup_array.at(GSLNodeIdToGSLIndex(target_node_id)) = distance;
}

void ArbiterDynamic::SetDestinationSatelliteList(std::array<std::set<int32_t>, ArbiterDynamic::NUM_GROUND_STATIONS> *dsl) {
    m_destination_satellite_list = *dsl;
}

std::string ArbiterDynamic::StringReprOfForwardingState() {
