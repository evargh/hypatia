...
TypeId GSLChannel::GetTypeId(void)
{
	static TypeId tid =
		TypeId("ns3::GSLChannel")
			.SetParent<Channel>()
			.SetGroupName("GSL")
			.AddConstructor<GSLChannel>()
			.AddAttribute("Delay",
						  "The lower-bound propagation delay through the channel (it is accessed by the distributed "
						  "simulator to determine lookahead time)",
						  TimeValue(Seconds(0)), MakeTimeAccessor(&GSLChannel::m_lowerBoundDelay), MakeTimeChecker())
			.AddAttribute(
				"PropagationSpeed", "Propagation speed through the channel in m/s (default is the speed of light)",
				DoubleValue(299792458.0), // Default is speed of light
				MakeDoubleAccessor(&GSLChannel::m_propagationSpeedMetersPerSecond), MakeDoubleChecker<double>());
	return tid;
}

GSLChannel::GSLChannel() : Channel()
{
	NS_LOG_FUNCTION_NOARGS();
}

bool GSLChannel::TransmitStart(Ptr<const Packet> p, Ptr<GSLNetDevice> src, Address dst_address, Time txTime)
{
...
bool GSLChannel::TransmitTo(Ptr<const Packet> p, Ptr<GSLNetDevice> srcNetDevice, Ptr<GSLNetDevice> destNetDevice,
							Time txTime, bool isSameSystem)
{
	...
	{
		Simulator::ScheduleWithContext(receiverNode->GetId(), txTime + delay, &GSLNetDevice::Receive, destNetDevice,
									   p->Copy());
	}
	else
	{
	...
	//    MpiInterface::SendPacket (p->Copy (), rxTime, destNetDevice->GetNode()->GetId (),
	//    destNetDevice->GetIfIndex());
	// #else
...
void GSLChannel::Attach(Ptr<GSLNetDevice> device)
{
...
Time GSLChannel::GetDelay(Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
...
std::size_t GSLChannel::GetNDevices(void) const
{
	NS_LOG_FUNCTION_NOARGS();
	return m_net_devices.size();
}

Ptr<NetDevice> GSLChannel::GetDevice(std::size_t i) const
{
