...
TypeId ArbiterShortSat::GetTypeId(void) {
  static TypeId tid = TypeId("ns3::ArbiterShortSat")
                          .SetParent<ArbiterSatnet>()
                          .SetGroupName("BasicSim");
  return tid;
}

ArbiterShortSat::ArbiterShortSat(
    ...
    : ArbiterSatnet(this_node, nodes), num_orbits(n_o),
...
ArbiterShortSat::HandleClose(int16_t destination_alpha,
                             int16_t destination_gamma,
                             int32_t target_node_id) {
  if (neighbors.CheckIfAlphaIncrease(NeighborCoordContainer::SELF,
                                     destination_alpha) == 0) {
    ...
    // further up again
    if (neighbors.CheckIfGammaIncrease(NeighborCoordContainer::SELF,
                                       destination_gamma) == 0) {
      NS_ASSERT_MSG(false, "Dropped inaccessible ground station: "
                               << m_node_id << " " << target_node_id);
      return std::make_tuple(-1, -1, -1);
    }
    if (neighbors.CheckIfGammaIncrease(NeighborCoordContainer::SELF,
                                       destination_gamma) == 1) {
      if (neighbors.VerifyInRange(NeighborCoordContainer::RIGHT,
                                  destination_alpha, destination_gamma))
        return m_neighbor_ids[3]; // go right (interface 4)
      else if (neighbors.VerifyInRange(NeighborCoordContainer::UP,
                                       destination_alpha, destination_gamma))
    ...
    if (neighbors.CheckIfGammaIncrease(NeighborCoordContainer::SELF,
                                       destination_gamma) == -1) {
      if (neighbors.VerifyInRange(NeighborCoordContainer::LEFT,
                                  destination_alpha, destination_gamma))
        return m_neighbor_ids[0]; // go left (interface 1)
      else if (neighbors.VerifyInRange(NeighborCoordContainer::DOWN,
                                       destination_alpha, destination_gamma))
  ...
  if (neighbors.CheckIfAlphaIncrease(NeighborCoordContainer::SELF,
    ...
    if (neighbors.CheckIfGammaIncrease(NeighborCoordContainer::SELF,
                                       destination_gamma) == -1) {
      if (neighbors.VerifyInRange(NeighborCoordContainer::DOWN,
                                  destination_alpha, destination_gamma))
        return m_neighbor_ids[1]; // go down
      else if (neighbors.VerifyInRange(NeighborCoordContainer::RIGHT,
                                       destination_alpha, destination_gamma))
  ...
  if (neighbors.CheckIfAlphaIncrease(NeighborCoordContainer::SELF,
    ...
    if (neighbors.CheckIfGammaIncrease(NeighborCoordContainer::SELF,
                                       destination_gamma) == 1) {
      if (neighbors.VerifyInRange(NeighborCoordContainer::UP, destination_alpha,
                                  destination_gamma))
        return m_neighbor_ids[2]; // go up
      else if (neighbors.VerifyInRange(NeighborCoordContainer::LEFT,
                                       destination_alpha, destination_gamma))
        return m_neighbor_ids[0]; // go left if necessary
      else
        NS_ASSERT_MSG(
            false, "routing failure for above-left"); // one must be in range
    ...
    if (neighbors.CheckIfGammaIncrease(NeighborCoordContainer::SELF,
      ...
      if (neighbors.VerifyInRange(NeighborCoordContainer::LEFT,
        ...
        NS_ASSERT_MSG(
            false, "routing failure for below-left"); // one must be in range
                                                      // for small-scale phasing
    }
  }
...
std::tuple<int32_t, int32_t, int32_t>
ArbiterShortSat::DetermineInterface(int16_t destination_alpha,
                                    int16_t destination_gamma,
                                    int32_t target_node_id) {
  ...
  int32_t min_hops = std::min({right_hops, left_hops, up_hops, down_hops},
                              std::less<int32_t>());
...
void ArbiterShortSat::SetGSShortTable(
...
std::tuple<int32_t, int32_t, int32_t> ArbiterShortSat::ShortDecide(
    std::vector<std::tuple<int16_t, int16_t>> adjacent_satellites,
    int32_t target_node_id) {
  int min_position = 0;
  int16_t min_distance = -1;
  for (int i = 0; i < adjacent_satellites.size(); i++) {
    int16_t distance = neighbors.GetHopcount(
        NeighborCoordContainer::SELF, std::get<0>(adjacent_satellites.at(i)),
  ...
  return DetermineInterface(std::get<0>(adjacent_satellites.at(min_position)),
                            std::get<1>(adjacent_satellites.at(min_position)),
                            target_node_id);
}

std::tuple<int32_t, int32_t, int32_t>
ArbiterShortSat::TopologySatelliteNetworkDecide(
  ...
  std::vector<std::tuple<int32_t, std::tuple<double, double>>>
      adjacent_satellites = m_other_table.at(
          target_node_id - num_orbits * num_satellites_per_orbit);
  std::vector<std::tuple<int16_t, int16_t>> adjacent_satellites_cells;
  for (auto elem : adjacent_satellites) {
    adjacent_satellites_cells.push_back(std::make_tuple(
        neighbors.CreateAlphaCell(std::get<0>(std::get<1>(elem))),
...
void ArbiterShortSat::SetSingleForwardState(int32_t target_node_id,
