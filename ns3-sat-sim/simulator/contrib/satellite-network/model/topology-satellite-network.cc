...
{
	static TypeId tid = TypeId("ns3::TopologySatelliteNetwork").SetParent<Object>().SetGroupName("SatelliteNetwork");
	return tid;
}
...
TopologySatelliteNetwork::TopologySatelliteNetwork(Ptr<BasicSimulation> basicSimulation,
												   const Ipv4RoutingHelper &ipv4RoutingHelper,
												   PointToPointLaserHelper p2pLaserHelper, GSLHelper gslHelper)
{
	m_basicSimulation = basicSimulation;
	ReadConfig();
...
TopologySatelliteNetwork::TopologySatelliteNetwork(Ptr<BasicSimulation> basicSimulation,
												   const Ipv4RoutingHelper &ipv4RoutingHelper,
												   DhpbPointToPointLaserHelper p2pLaserHelper, DhpbGSLHelper gslHelper)
{
	m_basicSimulation = basicSimulation;
	ReadConfig();
	Build(ipv4RoutingHelper, p2pLaserHelper, gslHelper);
...
void TopologySatelliteNetwork::ReadConfig()
{
	m_satellite_network_dir =
		m_basicSimulation->GetRunDir() + "/" + m_basicSimulation->GetConfigParamOrFail("satellite_network_dir");
	m_satellite_network_routes_dir =
		m_basicSimulation->GetRunDir() + "/" + m_basicSimulation->GetConfigParamOrFail("satellite_network_routes_dir");
	m_satellite_network_force_static =
		parse_boolean(m_basicSimulation->GetConfigParamOrDefault("satellite_network_force_static", "false"));
}

void TopologySatelliteNetwork::Build(const Ipv4RoutingHelper &ipv4RoutingHelper, PointToPointLaserHelper p2pLaserHelper,
									 GSLHelper gslHelper)
{
	...
	// Link settings
	m_isl_data_rate_megabit_per_s =
		parse_positive_double(m_basicSimulation->GetConfigParamOrFail("isl_data_rate_megabit_per_s"));
	m_gsl_data_rate_megabit_per_s =
		parse_positive_double(m_basicSimulation->GetConfigParamOrFail("gsl_data_rate_megabit_per_s"));
	m_isl_max_queue_size_pkts =
		parse_positive_int64(m_basicSimulation->GetConfigParamOrFail("isl_max_queue_size_pkts"));
	m_gsl_max_queue_size_pkts =
		parse_positive_int64(m_basicSimulation->GetConfigParamOrFail("gsl_max_queue_size_pkts"));

	// Utilization tracking settings
	m_enable_isl_utilization_tracking =
		parse_boolean(m_basicSimulation->GetConfigParamOrFail("enable_isl_utilization_tracking"));
	if (m_enable_isl_utilization_tracking)
	{
		m_isl_utilization_tracking_interval_ns =
			parse_positive_int64(m_basicSimulation->GetConfigParamOrFail("isl_utilization_tracking_interval_ns"));
	}
...
void TopologySatelliteNetwork::Build(const Ipv4RoutingHelper &ipv4RoutingHelper,
									 DhpbPointToPointLaserHelper p2pLaserHelper, DhpbGSLHelper gslHelper)
{
	std::cout << "SATELLITE NETWORK" << std::endl;
...
void TopologySatelliteNetwork::ReadSatellites()
{
	...
	while (std::getline(fs, name))
	{
		...
		else
		{
			...
			// Dynamic
			mobility.SetMobilityModel("ns3::SatellitePositionMobilityModel", "SatellitePositionHelper",
...
void TopologySatelliteNetwork::ReadGroundStations()
{
	...
	while (std::getline(fs, line))
	{
		...
		Ptr<GroundStation> gs =
			CreateObject<GroundStation>(gid, name, latitude, longitude, elevation, cartesian_position);
...
void TopologySatelliteNetwork::InstallInternetStacks(const Ipv4RoutingHelper &ipv4RoutingHelper)
{
...
void TopologySatelliteNetwork::ReadISLs(PointToPointLaserHelper p2p_laser_helper)
{
	// Link helper
	...
	p2p_laser_helper.SetDeviceAttribute(
		"DataRate", DataRateValue(DataRate(std::to_string(m_isl_data_rate_megabit_per_s) + "Mbps")));
	...
	while (std::getline(fs, line))
	{
		...
		if (m_enable_isl_utilization_tracking)
		{
			netDevices.Get(0)->GetObject<PointToPointLaserNetDevice>()->EnableUtilizationTracking(
				m_isl_utilization_tracking_interval_ns);
			netDevices.Get(1)->GetObject<PointToPointLaserNetDevice>()->EnableUtilizationTracking(
				m_isl_utilization_tracking_interval_ns);
...
void TopologySatelliteNetwork::ReadISLs(DhpbPointToPointLaserHelper p2p_laser_helper)
{
	// Link helper
	std::string max_queue_size_str = format_string("%" PRId64 "p", m_isl_max_queue_size_pkts);
	p2p_laser_helper.SetQueue("ns3::DropTailQueue<Packet>", "MaxSize", QueueSizeValue(QueueSize(max_queue_size_str)));
	...
	while (std::getline(fs, line))
	{
		...
		if (m_enable_isl_utilization_tracking)
		{
			netDevices.Get(0)->GetObject<DhpbPointToPointLaserNetDevice>()->EnableUtilizationTracking(
				m_isl_utilization_tracking_interval_ns);
			netDevices.Get(1)->GetObject<DhpbPointToPointLaserNetDevice>()->EnableUtilizationTracking(
				m_isl_utilization_tracking_interval_ns);
...
void TopologySatelliteNetwork::CreateGSLs(GSLHelper gsl_helper)
{
	...
	gsl_helper.SetDeviceAttribute("DataRate",
								  DataRateValue(DataRate(std::to_string(m_gsl_data_rate_megabit_per_s) + "Mbps")));
	...
	std::cout << "    >> Read all GSL interfaces information for the " << node_gsl_if_info.size() << " nodes"
			  << std::endl;
	...
	std::cout << "    >> Finished install GSL interfaces (interfaces, network devices, one shared channel)"
			  << std::endl;
	...
	for (uint32_t i = 0; i < devices.GetN(); i++)
	{
		...
		if (((i + 1) % update_interval) == 0 || (i + 1) == devices.GetN())
		{
			...
			printf("       - %.2f%% (t = %.2f s, update took %.2f s)\n", (float)(i + 1) / (float)devices.GetN() * 100.0,
				   (now_ns - start_time_ns) / 1e9, (now_ns - last_time_ns) / 1e9);
			last_time_ns = now_ns;
		}
	}
...
void TopologySatelliteNetwork::CreateGSLs(DhpbGSLHelper gsl_helper)
{
...
void TopologySatelliteNetwork::PopulateArpCaches()
{
	...
	for (uint32_t i = 0; i < m_allNodes.GetN(); i++)
	{
		...
		for (size_t j = 1; j < m_allNodes.Get(i)->GetObject<Ipv4>()->GetNInterfaces(); j++)
		{
			Mac48Address mac48Address =
				Mac48Address::ConvertFrom(m_allNodes.Get(i)->GetObject<Ipv4>()->GetNetDevice(j)->GetAddress());
			...
			// pointer)
			m_allNodes.Get(i)->GetObject<Ipv4L3Protocol>()->GetInterface(j)->SetAttribute("ArpCache",
																						  PointerValue(arpAll));
		}
	}
}
		...
		for (size_t i = 0; i < m_islNetDevices.GetN(); i++)
		{
			...
			for (size_t j = 0; j < utilization.size(); j++)
			{
				...
				if (j == utilization.size() - 1 || utilization[j] != utilization[j + 1])
				{
					...
					fprintf(file_utilization_csv, "%d,%d,%" PRId64 ",%" PRId64 ",%f\n", src_dst.first, src_dst.second,
							interval_left_side_ns, (j + 1) * m_isl_utilization_tracking_interval_ns, utilization[j]);
