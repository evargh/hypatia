...
namespace ns3 {
    ...
    TopologySatelliteNetwork::TopologySatelliteNetwork(Ptr<BasicSimulation> basicSimulation, const Ipv4RoutingHelper& ipv4RoutingHelper, DhpbPointToPointLaserHelper p2pLaserHelper, DhpbGSLHelper gslHelper) {
        m_basicSimulation = basicSimulation;
        ReadConfig();
        Build(ipv4RoutingHelper, p2pLaserHelper, gslHelper);
    }
    ...
    void
    TopologySatelliteNetwork::Build(const Ipv4RoutingHelper& ipv4RoutingHelper, DhpbPointToPointLaserHelper p2pLaserHelper, DhpbGSLHelper gslHelper) {
        std::cout << "SATELLITE NETWORK" << std::endl;
    ...
    void
    TopologySatelliteNetwork::ReadISLs(DhpbPointToPointLaserHelper p2p_laser_helper)
    {
        // Link helper
        std::string max_queue_size_str = format_string("%" PRId64 "p", m_isl_max_queue_size_pkts);
        p2p_laser_helper.SetQueue("ns3::DropTailQueue<Packet>", "MaxSize", QueueSizeValue(QueueSize(max_queue_size_str)));
        ...
        while (std::getline(fs, line)) {
            ...
            // Utilization tracking
            if (m_enable_isl_utilization_tracking) {
                netDevices.Get(0)->GetObject<DhpbPointToPointLaserNetDevice>()->EnableUtilizationTracking(m_isl_utilization_tracking_interval_ns);
                netDevices.Get(1)->GetObject<DhpbPointToPointLaserNetDevice>()->EnableUtilizationTracking(m_isl_utilization_tracking_interval_ns);
    ...
    void
    TopologySatelliteNetwork::CreateGSLs(DhpbGSLHelper gsl_helper) {
