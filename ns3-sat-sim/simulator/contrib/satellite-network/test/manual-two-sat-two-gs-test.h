...
class ManualTwoSatTwoGsTest : public TestCase
{
  public:
	...
	void setup_scenario(double distance_multiplier, bool new_prop_speed, double new_prop_speed_m_per_s)
	{
		...
		satelliteNodes.Get(0)->GetObject<MobilityModel>()->SetPosition(
			Vector(-1 * distance_multiplier, -1 * distance_multiplier, -1 * distance_multiplier));
		mobility.Install(satelliteNodes.Get(1));
		satelliteNodes.Get(1)->GetObject<MobilityModel>()->SetPosition(
			Vector(1 * distance_multiplier, -1 * distance_multiplier, -1 * distance_multiplier));

		// Ground stations mobility models
		mobility.Install(groundStationNodes.Get(0));
		groundStationNodes.Get(0)->GetObject<MobilityModel>()->SetPosition(
			Vector(1 * distance_multiplier, 1 * distance_multiplier, -1 * distance_multiplier));
		mobility.Install(groundStationNodes.Get(1));
		groundStationNodes.Get(1)->GetObject<MobilityModel>()->SetPosition(
			Vector(1 * distance_multiplier, -1 * distance_multiplier, 1 * distance_multiplier));
		...
		tch_isl.SetRootQueueDisc("ns3::FifoQueueDisc", "MaxSize",
								 QueueSizeValue(QueueSize("1p"))); // Will be removed later any case
		...
		tch_gsl.SetRootQueueDisc("ns3::FifoQueueDisc", "MaxSize",
								 QueueSizeValue(QueueSize("1p"))); // Will be removed later any case
		...
		NetDeviceContainer devices = gsl_helper.Install(satelliteNodes, groundStationNodes, node_gsl_if_info);
		...
		ASSERT_EQUAL(
			4,
			allNodes.Get(2)->GetObject<Ipv4>()->GetNetDevice(1)->GetChannel()->GetObject<GSLChannel>()->GetNDevices());
		ASSERT_EQUAL(
			devices.Get(1),
			allNodes.Get(2)->GetObject<Ipv4>()->GetNetDevice(1)->GetChannel()->GetObject<GSLChannel>()->GetDevice(1));
		...
		for (uint32_t i = 0; i < allNodes.GetN(); i++)
		{
			...
			for (size_t j = 1; j < allNodes.Get(i)->GetObject<Ipv4>()->GetNInterfaces(); j++)
			{
				Mac48Address mac48Address =
					Mac48Address::ConvertFrom(allNodes.Get(i)->GetObject<Ipv4>()->GetNetDevice(j)->GetAddress());
				...
				// Set a pointer to the ARP cache it should use (will be filled at the end of this function, it's only a
				// pointer)
				allNodes.Get(i)->GetObject<Ipv4L3Protocol>()->GetInterface(j)->SetAttribute("ArpCache",
																							PointerValue(arpAll));
			}
		}
	}
  ...
  public:
						 ...
						 ns3::Ipv4Header const &ipHeader, bool is_socket_request_for_source_ip)
	{
		...
		if (next_node_id != -1)
		{
			...
			uint32_t select_ip_gateway =
				m_nodes.Get(next_node_id)->GetObject<Ipv4>()->GetAddress(next_if_id, 0).GetLocal().Get();
...
class ManualTwoSatTwoGsFirstTest : public ManualTwoSatTwoGsTest
{
  public:
	...
	void DoRun()
	{
		...
		UdpBurstInfo udpBurstInfo(0, 2, 3,
								  3, // Rate in Mbit/s
								  0,
		...
		udpApp.Get(2)->GetObject<UdpBurstApplication>()->RegisterOutgoingBurst(
			udpBurstInfo, InetSocketAddress(allNodes.Get(3)->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal(), 1026),
			true);
		udpApp.Get(3)->GetObject<UdpBurstApplication>()->RegisterIncomingBurst(udpBurstInfo, true);

		// Run simulation
		basicSimulation->Run();

		// Check UDP burst completion information
		std::vector<std::tuple<UdpBurstInfo, uint64_t>> outgoing_2_info =
			udpApp.Get(2)->GetObject<UdpBurstApplication>()->GetOutgoingBurstsInformation();
		std::vector<std::tuple<UdpBurstInfo, uint64_t>> outgoing_3_info =
			udpApp.Get(3)->GetObject<UdpBurstApplication>()->GetOutgoingBurstsInformation();
		std::vector<std::tuple<UdpBurstInfo, uint64_t>> incoming_2_info =
			udpApp.Get(2)->GetObject<UdpBurstApplication>()->GetIncomingBurstsInformation();
		std::vector<std::tuple<UdpBurstInfo, uint64_t>> incoming_3_info =
			udpApp.Get(3)->GetObject<UdpBurstApplication>()->GetIncomingBurstsInformation();
		...
		// Outgoing
		std::vector<std::string> lines_precise_outgoing_csv =
			read_file_direct(temp_dir + "/logs_ns3/udp_burst_0_outgoing.csv");
		...
		// Incoming
		std::vector<std::string> lines_precise_incoming_csv =
			read_file_direct(temp_dir + "/logs_ns3/udp_burst_0_incoming.csv");
		...
		{
			std::vector<std::string> line_spl = split_string(line, ",");
			double hop_a_distance_m = std::sqrt(std::pow(1000000 - (-1000000), 2) + std::pow(1000000 - (-1000000), 2) +
												std::pow(-1000000 - (-1000000), 2));
			double hop_a_latency_ns = hop_a_distance_m / (299792458.0 / 1000000000.0);

			double hop_b_distance_m = std::sqrt(std::pow(-1000000 - (1000000), 2) + std::pow(-1000000 - (-1000000), 2) +
												std::pow(-1000000 - (-1000000), 2));
			double hop_b_latency_ns = hop_b_distance_m / (299792458.0 / 1000000000.0);

			double hop_c_distance_m = std::sqrt(std::pow(1000000 - (1000000), 2) + std::pow(-1000000 - (-1000000), 2) +
												std::pow(-1000000 - (1000000), 2));
			...
			// std::cout << " Hop A: " << hop_a_distance_m << " m gives latency " << hop_a_latency_ns << " ns" <<
			// std::endl; std::cout << " Hop B: " << hop_b_distance_m << " m gives latency " << hop_b_latency_ns << "
			// ns" << std::endl; std::cout << " Hop C: " << hop_c_distance_m << " m gives latency " << hop_c_latency_ns
			// << " ns" << std::endl;

			// At most 10 nanoseconds off due to rounding on the way
			ASSERT_EQUAL_APPROX(parse_positive_int64(line_spl[2]) - sent_timestamps[j],
								time_one_way_latency_ns + (1502 / (0.000125 * 7.0)) + (1502.0 / (0.000125 * 4.0)) +
									(1502.0 / (0.000125 * 7.0)),
								10);
...
class ManualTwoSatTwoGsUpSharedTest : public ManualTwoSatTwoGsTest
{
  public:
	...
	void DoRun()
	{
		...
		int src_udp_id = 3;
		int dst_udp_id = 2;
		UdpBurstInfo udpBurstInfo(0, src_udp_id, dst_udp_id,
								  2, // Rate in Mbit/s
								  0,
								  100000000000, // Duration in ns // 100000000000
								  "abc", "def");
		...
		// Register all bursts being sent from there and being received
		udpApp.Get(src_udp_id)
			->GetObject<UdpBurstApplication>()
			->RegisterOutgoingBurst(
				udpBurstInfo,
				InetSocketAddress(allNodes.Get(dst_udp_id)->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal(), 1026),
				true);
		udpApp.Get(dst_udp_id)->GetObject<UdpBurstApplication>()->RegisterIncomingBurst(udpBurstInfo, true);
		...
		TcpFlowSendHelper source0(
			...
			100000000, 0, true, basicSimulation->GetLogsDir(), "");
		...
		std::cout << "TCP rate: " << app.Get(0)->GetObject<TcpFlowSendApplication>()->GetAckedBytes() / 2.0 / 125000.0
				  << std::endl;

		// And the UDP flow around 2 Mbit/s
		std::vector<std::tuple<UdpBurstInfo, uint64_t>> incoming_3_info =
			udpApp.Get(dst_udp_id)->GetObject<UdpBurstApplication>()->GetIncomingBurstsInformation();
		std::cout << "UDP rate: " << (double)std::get<1>(incoming_3_info.at(0)) * 1500.0 / 2.0 / 125000.0 << std::endl;

		// And the tests
		ASSERT_EQUAL_APPROX(app.Get(0)->GetObject<TcpFlowSendApplication>()->GetAckedBytes() / 2.0 / 125000.0, 5.0,
							1.0);
  ...
  public:
	...
	void DoRun()
	{
		...
		for (size_t i = 0; i < test_configs.size(); i++)
		{
			...
			arbiter = CreateObject<ArbiterCustom2>(allNodes.Get(0), allNodes);
			allNodes.Get(0)->GetObject<Ipv4>()->GetRoutingProtocol()->GetObject<Ipv4ArbiterRouting>()->SetArbiter(
				arbiter);

			arbiter = CreateObject<ArbiterCustom2>(allNodes.Get(1), allNodes);
			allNodes.Get(1)->GetObject<Ipv4>()->GetRoutingProtocol()->GetObject<Ipv4ArbiterRouting>()->SetArbiter(
				arbiter);

			arbiter = CreateObject<ArbiterCustom2>(allNodes.Get(2), allNodes);
			allNodes.Get(2)->GetObject<Ipv4>()->GetRoutingProtocol()->GetObject<Ipv4ArbiterRouting>()->SetArbiter(
				arbiter);

			arbiter = CreateObject<ArbiterCustom2>(allNodes.Get(3), allNodes);
			allNodes.Get(3)->GetObject<Ipv4>()->GetRoutingProtocol()->GetObject<Ipv4ArbiterRouting>()->SetArbiter(
				arbiter);
			...
			// UDP burst info entry
			UdpBurstInfo udpBurstInfo1(0, src_udp_id_1, dst_udp_id_1,
									   burst_1_rate, // Rate in Mbit/s
									   0,
									   100000000000, // Duration in ns // 100000000000
									   "abc", "def");
			udpApp.Get(src_udp_id_1)
				->GetObject<UdpBurstApplication>()
				->RegisterOutgoingBurst(
					udpBurstInfo1,
					InetSocketAddress(allNodes.Get(dst_udp_id_1)->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal(), 1026),
					true);
			udpApp.Get(dst_udp_id_1)->GetObject<UdpBurstApplication>()->RegisterIncomingBurst(udpBurstInfo1, true);

			// UDP burst info entry
			UdpBurstInfo udpBurstInfo2(1, src_udp_id_2, dst_udp_id_2,
									   burst_2_rate, // Rate in Mbit/s
									   0,
									   100000000000, // Duration in ns // 100000000000
									   "abc", "def");
			udpApp.Get(src_udp_id_2)
				->GetObject<UdpBurstApplication>()
				->RegisterOutgoingBurst(
					udpBurstInfo2,
					InetSocketAddress(allNodes.Get(dst_udp_id_2)->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal(), 1026),
					true);
			udpApp.Get(dst_udp_id_2)->GetObject<UdpBurstApplication>()->RegisterIncomingBurst(udpBurstInfo2, true);

			// Run simulation
			basicSimulation->Run();

			// And the UDP flows should have half of the 7 Mbit/s up of the ground station each
			std::vector<std::tuple<UdpBurstInfo, uint64_t>> incoming_1_info =
				udpApp.Get(dst_udp_id_1)->GetObject<UdpBurstApplication>()->GetIncomingBurstsInformation();
			std::vector<std::tuple<UdpBurstInfo, uint64_t>> incoming_2_info =
				udpApp.Get(dst_udp_id_2)->GetObject<UdpBurstApplication>()->GetIncomingBurstsInformation();
			size_t index_2 = 0;
  ...
  public:
	...
	void DoRun()
	{
		...
		TcpFlowSendHelper source0(
			"ns3::TcpSocketFactory",
			InetSocketAddress(allNodes.Get(dst_tcp_id)->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal(), 1024),
			10000000000, 0, true, basicSimulation->GetLogsDir(), "");
		app = source0.Install(allNodes.Get(src_tcp_id));
		app.Start(NanoSeconds(0));
		app.Stop(NanoSeconds(10000000000));
		...
		// UDP info
		std::vector<std::tuple<UdpBurstInfo, uint64_t>> incoming_udp_info =
			udpApp.Get(dst_udp_id)->GetObject<UdpBurstApplication>()->GetIncomingBurstsInformation();

		//        std::cout << "TCP Rate:" << app.Get(0)->GetObject<TcpFlowSendApplication>()->GetAckedBytes() / 2.0 /
		//        125000.0 << std::endl; std::cout << "UDP Rate:" << (double) std::get<1>(incoming_2_info.at(0)) *
		//        1500.0 / 1.0 / 125000.0  << std::endl;

		// TCP flow should have about 5.5-7 Mbit/s (TCP is not great)
		ASSERT_EQUAL_APPROX(app.Get(0)->GetObject<TcpFlowSendApplication>()->GetAckedBytes() / 2.0 / 125000.0, 7.0,
							1.5);
...
class ManualTwoSatTwoGsChangingForwardingTest : public ManualTwoSatTwoGsTest
{
  public:
	...
	void DoRun()
	{
		...
		Ptr<Arbiter> arbiter =
			allNodes.Get(2)->GetObject<Ipv4>()->GetRoutingProtocol()->GetObject<Ipv4ArbiterRouting>()->GetArbiter();

		// At the start
		ASSERT_EQUAL(arbiter->GetObject<ArbiterSingleForward>()->StringReprOfForwardingState(),
...
class ManualTwoSatTwoGsChangingRateTest : public ManualTwoSatTwoGsTest
{
  public:
	...
	void DoRun()
	{
		...
		ASSERT_EQUAL_APPROX(arrival_1s_to_2s, (2.8 / 4.0) * 100.0 + 100.0,
							5); // 100 packets are still in the GSL queue, and then the ISL queue is put into the GSL
								// queue at 2.8 Mbit/s (losing 1.2 Mbit/s)
