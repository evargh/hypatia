...
2. In `hypatia/ns3-sat-sim/`, edit `build.sh` and add `--disable-werror` (if it's not already there) to each `./waf configure <arguments>`. This is because modern versions of g++ produce a warning for a program in the submodule, which stops the build.
...
5. Now, outside of the container, run `git submodule update --init --recursive`.
6. In the container, execute `hypatia_build.sh,` and when that's done, execute `hypatia_run_tests.sh`.
