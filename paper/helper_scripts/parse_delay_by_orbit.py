    ...
    for line in file:
        ...
        if len(splitline) > 2:
            ...
            if uidphrase[0] == "UID":
                ...
                context = line_data[0].strip()  # what class is this
                result = line_data[1].strip()  # what
        ...
        for item in values:
            ...
            if label == "TransmitGSL":
                if from_id >= int(args.orbits * args.satellites_per_orbit):
                    ...
                    if check_blank_deque(
                        start_gsl_queues, from_id
                    ) and check_blank_deque(receive_queues, from_id):
                        ttl_fails.add(key)
                        print(
                            f"no populated queues for TransmitGSL, assuming TTL error",
                            file=sys.stderr,
                        )

                    elif check_blank_deque(
                        start_gsl_queues, from_id
                    ) and not check_blank_deque(receive_queues, from_id):
                        test = pop_appropriate_deque(receive_queues, from_id, key)
                        if test is None:
                            sys.exit("GSL: popping from blank receive queue")

                    elif not check_blank_deque(
                        start_gsl_queues, from_id
                    ) and check_blank_deque(receive_queues, from_id):
                        ...
                        if receive_queues[from_id][0] < start_gsl_queues[from_id][0]:
                            ...
                            if test is None:
                                sys.exit(
                                    "GSL: both queues full, but popped blank receive queue"
                                )
                        else:
                            test = pop_appropriate_deque(start_gsl_queues, from_id, key)
                            if test is None:
                                sys.exit(
                                    "GSL: both queues full, but popped blank gsl queue"
                                )
                ...
                minval = None
                if check_blank_deque(start_gsl_queues, from_id) and check_blank_deque(
                    receive_queues, from_id
                ):
                    ttl_fails.add(key)
                    print(
                        f"no populated queues for transmitISL, assuming TTL error",
                        file=sys.stderr,
                    )
                    ...
                    if receive_queues[from_id][0] < start_gsl_queues[from_id][0]:
                        ...
                        if minval is None:
                            sys.exit(
                                "ISL: both queues full, but popped blank receive queue"
                            )
                        else:
                            if this_orbit not in timediffs:
                                timediffs[this_orbit] = []
                        ...
                        if test is None:
                            sys.exit(
                                "ISL: both queues full, but popped blank gsl queue"
                            )
            else:
