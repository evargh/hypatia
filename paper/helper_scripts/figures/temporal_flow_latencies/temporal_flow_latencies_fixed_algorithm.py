...
dataset = [
    extract_percentile(args.file, args.percentiles[i] / 100.0)
    for i in range(len(args.percentiles))
]
...
ax.set_title(
    f"Time for Flows to Complete in Starlink Over Time"
...
ax.set_xlim(-1, 175)
ax.legend()
plt.savefig("temporal_flow_latencies_fixed_algorithm.png")
